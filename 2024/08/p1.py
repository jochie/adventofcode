    ...
    for freq, positions in antennas.items():
        for row1, col1 in positions:
            for row2, col2 in positions:
                ...
                anti1_row = row1 - row_diff
                anti1_col = col1 - col_diff
                if on_grid(anti1_row, anti1_col, max_row, max_col):
                    antinodes[f"({anti1_row},{anti1_col})"] = True

                anti2_row = row2 + row_diff
                anti2_col = col2 + col_diff
                if on_grid(anti2_row, anti2_col, max_row, max_col):
