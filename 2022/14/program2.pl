...
sub simulate_grain {
    ...
    if ($positions{"500,0"} eq "o") {
	return 0;
    }
    while (1) {
	if ($g_y == $max_y + 1) {
	    # Simulate the infinite floor, two below the lowest point
	    $positions{"$g_x,$g_y"} = "o";
	    return 1;
	}
...
while (1) {
    if (!simulate_grain()) {
	print "Grain is blocking the starting point after $grains grains\n";
	last;
    }
    print "\n";
    $grains++;
