...
fn validate_number(matches: &Matches, number: i32) -> bool {
    let mut last_digit = number % 10;
    let mut checking = number / 10;
    let mut found_adj_same = false;

    while checking > 0 {
        let next_digit = checking % 10;
        if next_digit > last_digit {
            // From left to right, the digits never decrease, meaning
            // from right to left, the digits never increase
            return false;
        }
    ...
    if found_adj_same && matches.opt_present("v") {
        println!("Potential password: {number}");
    }
    found_adj_same
}
...
fn validate_number2(matches: &Matches, number: i32) -> bool {
    let mut last_digit = number % 10;
    let mut checking = number / 10;
    let mut sequence = 1;
    let mut found_adj_same = false;

    // This time, track how many matching digits there were in a row,
    // either in the middle of the number or at the end
    while checking > 0 {
        let next_digit = checking % 10;
        if next_digit > last_digit {
            // From left to right, the digits never decrease, meaning
