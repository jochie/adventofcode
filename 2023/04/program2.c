...
process_file(FILE *fd)
{
        ...
        *(bar - 1) = '\0';
        ...
        if (hits > 0) {
            for (int i = 0; i < hits; i++) {
                if (opts.debug) {
                    printf("Adding %d copies of card %d\n", allcards[card - 1], card + i + 1);
                }
                allcards[card - 1 + i + 1] += allcards[card - 1];
            }
        }
        free(winners_list);
        free(numbers_list);
    ...
    printf("Final total of %d scratchcards\n", sum);
    if (opts.debug) {
...
print_usage(FILE *f, char *argv0, char *prefix, bool full, int exitcode)
{
    ...
    fprintf(f, "\
NAME\n\
     %s - Program for AoC 2023 puzzles; Day 4, part 2\n\
\n\
SYNOPSIS\n\
     %s [OPTIONS] [<filename> ...]\n",
            name, name);
...
\n\
DESCRIPTION\n\
     This program is used for one of the AoC 2023 puzzles; Day 4, part 2.\n\
